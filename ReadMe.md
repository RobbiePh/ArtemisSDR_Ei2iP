...
- **"S-ATT 31 in red" cosmetic warning fixed.** The main-window ATT label flipping to `[S-ATT]` with a permanent red `31` value during TX was an Anan-era leftover. The 0-31 dB "step attenuator" is an Alex-board hardware register that SunSDR2 DX doesn't have — the SunSDR's only attenuator is the +10/0/-10/-20 cycle. The "31 in red" was triggered by the Anan-specific "Force ATT on Tx to 31 when PS-A is off" rule firing on SunSDR (where PS-A is hardware-disabled, so the rule's condition is always true). The displayed 31 never actually reached the radio in any meaningful way — purely a misleading UI artifact. Now hard-disabled on SunSDR: the main-UI ATT label stays `S-ATT` / `ATT` (no brackets, no red 31) regardless of any saved-DB ATT-on-TX state. AGC dropdown and the actual ATT cycle button are unaffected.
